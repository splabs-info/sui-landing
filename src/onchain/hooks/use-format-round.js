...
export const useFormatRound = () => {
  ...
  const formatInfoRound = React.useCallback(
    async (name, projectName) => {
      ...
      const rounds = currentProject
        .map((item) => item?.all_rounds?.find((round) => round?.name === name))
        .filter(Boolean);
      ...
      if (Array.isArray(results) && results.length > 0) {
        ...
        const corePromises = nonNullResults.flatMap((item) =>
          Object.values(item).flatMap((ele) => Object.values(ele).flatMap((e) => fetchDynamicFieldObject(e)))
        );
        let cores;
        try {
        ...
        policies.forEach((p) => {
          ...
          additionalInfo = {
            ...additionalInfo,
            maxPurchase: Number(ethers.utils.formatUnits(p?.max_purchase, rounds[0]?.token_decimal)) || 0,
            minPurchase: Number(ethers.utils.formatUnits(p?.min_purchase, rounds[0]?.token_decimal)) || 0,
          };
        });
