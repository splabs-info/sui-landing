...
export default function CopyContent({
    ...
    return text ? (
        <Grid container width="100%">
            <Grid item xs={12}>
                <Stack direction={'row'} flexGrow={1} alignItems="center" justifyContent="space-between" {...other}>
                    <Stack
                        direction={'row'}
                        sx={{
                            cursor: 'pointer',
              ...
              <> */}
                        ...
                        <Typography variant="body">{text.slice(text.length - length, text.length)}</Typography>
                        {/* </>
            )} */}
                    </Stack>
                    {!hideButton && (
            ...
            {confirmMessage && (
                <Popup open={showConfirm} onClose={() => setShowConfirm(false)}>
                    <Stack alignItems="center" sx={{ width: 'cacl(90vw + 50px)', maxWidth: '450px' }}>
