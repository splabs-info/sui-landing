...
export default function PreviousPools({ previous }) {
    const isMobile = useResponsive('down', 'sm');
    const navigate = useNavigate();
    const [list, setList] = React.useState([dataSUA, dataReleapCommunity]);


    // React.useEffect(() => {
    //     if (!isEmpty(previous)) {
    //         // Hợp nhất danh sách hiện tại với previous
    //         setList(prevList => [...prevList, ...previous]);
    //     }
    // }, [previous]);

    console.log('list___', previous)
    // useEffect(() => {
    //     if (hasOutTimeIDOXUI) {
